...
minecoor = adapter.getMineCoor()
coor = adapter.getCoor()
move = adapter.getMoves()
moves = []

for mv in move:
    for c in coor:
        if (mv[1] == c[0]):
            moves.append((mv[0],c[1],c[2]))
...
def menu():
    ...
    if button.clickButton(200,250,100,50,RED,ORANGE,font,"EASY",BLACK):
        game.reset(int(sqrt(len(coor))),int(sqrt(len(coor))),len(minecoor))
        gameState = 0
...
class Tile():
    ...
    def update(self):
        global gameState
        if gameState == 0:
            if (mouse_state == 1 and mouse_x >= self.x and mouse_x <= (self.x + (size[0] / self.columns)) and mouse_y >= self.y and mouse_y <= (self.y + ((size[1] - 100) / self.rows))) or (moved == 1):
                self.visible = True
                self.flag = False
            if (mouse_state == 3 and mouse_x >= self.x and mouse_x <= (self.x + (size[0] / self.columns)) and mouse_y >= self.y and mouse_y <= (self.y + ((size[1] - 100) / self.rows))) or (moved == 3):
                if self.flag == False:
                    self.flag = True
                elif self.flag == True:
                    self.flag = False
            if self.visible == True and self.mine == True and moved != -1:
                print("masuk")
                gameState = 2
            if moved == -1:
                gameState = 1
...
class Game():
    ...
    def reset(self,columns,rows,mines):
        ...
        #placing mines
        for mine in minecoor:
            self.mineloc = [mine[0],mine[1]]
            if self.board[self.mineloc[1]][self.mineloc[0]].mine == False:
                self.mines.append(self.mineloc)
                self.board[self.mineloc[1]][self.mineloc[0]].mine = True
            self.minenum = len(self.mines)
...
while not done:
    ...
    if (len(moves) != 0):
        ...
        next_move = ("finish",-1,-1)
    if next_move[0] == "move":
        moved = 1
    if next_move[0] == "flag":
        moved = 3
    if next_move[0] == "finish":
        moved = -1
    move_x = next_move[1]
    move_y = next_move[2]
